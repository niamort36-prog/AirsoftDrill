...
auth.onAuthStateChanged(user => {
    if (user) {
        currentUser = user;
        document.getElementById('login-btn').innerText = `Déconnexion (${user.displayName || user.email})`;
...
document.getElementById('login-btn').addEventListener('click', () => {
    if (currentUser) {
        ...
        auth.signInWithPopup(provider).catch(error => alert("Erreur de connexion : " + error.message));
    }
});
...
document.getElementById('generate-drill-btn').addEventListener('click', async () => {
    ...
    - Matériel disponible : ${selectedMaterials.length > 0 ? selectedMaterials.join(', ') : 'Aucun spécifique'}
    - Sources documentaires : \n${docContext}
    - Terrain sélectionné : ${selectedTerrain.name} (${selectedTerrain.type})
    - Données topographiques en JSON : ${JSON.stringify(selectedTerrain.data)}
